require File.dirname(__FILE__) + '/../lib/akwire/io.rb'
require File.dirname(__FILE__) + '/helpers.rb'

describe 'Akwire::IO' do
  include Helpers

  it 'can execute a command' do
    output, status = Akwire::IO.popen('echo test')
    output.should eq("test\n")
    status.should eq(0)
  end

  it 'can execute a command with a non-zero exist status' do
    output, status = Akwire::IO.popen('echo test && exit 1')
    output.should eq("test\n")
    status.should eq(1)
  end

  it 'can execute an unknown command' do
    output, status = Akwire::IO.popen('unknown.command')
    output.should include('unknown')
    status.should eq(127)
  end

  it 'can time out command execution (ruby 1.9.3 only)' do
    output, status = Akwire::IO.popen('sleep 1 && echo -n "Ruby 1.8"', 'r', 0.25)
  ...
  it 'can execute a command and write to stdin' do
    ...
    output, status = Akwire::IO.popen('cat > ' + file_name, 'r+') do |child|
      child.write(content)
      child.close_write
    end
  ...
  it 'can execute a command asynchronously' do
    async_wrapper do
      Akwire::IO.async_popen('echo fail && exit 2') do |output, status|
  ...
  it 'can execute a command asynchronously and write to stdin' do
    ...
    async_wrapper do
      Akwire::IO.async_popen('cat > ' + file_name, timestamp) do |output, status|
        status.should eq(0)
        async_done
      end
    end
